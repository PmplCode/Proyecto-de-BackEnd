...
router.post(
  "/editarPerfil",
  isLoggedIn,
  fileUploader.single("perfil-cover-image"),
  (req, res, next) => {
...
router.post(
  "/crear",
  isLoggedIn,
  fileUploader.single("coctel-cover-image"),
  (req, res) => {
    ...
    alcohol.forEach((paraula, k) => {
      if (paraula.length > 0) paraules.push(capitalize(paraula));
    });
    Coctel.create({
      name,
      alcohol: paraules,
      ...
      creador: req.session.currentUser._id,
